...
const authorize = async () => {
  try {
    const apiKey = await remix.call(
      "settings",
      "get",
      "settings/sindri-access-token"
    )
    if (!apiKey) {
    ...
    const message =
      'No Sindri API key found. Please click the gear in the lower left corner to open the settings page, and add your API key under "Sindri Credentials".'
 ...
 */
export const compile = async (tags = ["latest"]) => {
  await authorize()
  ...
  const filesByPath = {}
  const workspace = await remix.call("fileManager", "copyFolderToJson", "/")
  ...
  if (sindriManifest.circuitType === "circom") {
    const circuitPath = normalizePath(
      sindriManifest.circuitPath || "circuit.circom"
    )
    let circuitContent
    try {
      ...
      console.error(
        `No circuit file found at "${circuitPath}", try setting "circuitPath" in "sindri.json".`
      )
    }
    const dependencies = await remix.call(
      "circuit-compiler",
      "resolveDependencies",
      circuitPath,
      circuitContent
    ...
    Object.entries(dependencies).forEach(([rawPath, rawContent]) => {
      ...
      const content = path.endsWith(".circom")
        ? rawContent.replace(
            /^\s*include\s+"\/+([^"]+)"\s*;\s*$/gm,
            'include "$1";'
          )
  ...
  try {
    ...
    if (circuitResponse.status === "Ready") {
      console.log(
        `Circuit compiled successfully, circuit id: ${circuitResponse.circuit_id}`
      )
    } else {
      console.error(
        "Circuit compilation failed:",
        circuitResponse.error || "Unknown error"
      )
    }
    ...
    if ("status" in error && error.status === 401) {
      const message =
        "Sindri API key authentication failed, please check that your key is correct in the settings."
 ...
 */
export const prove = async signals => {
  await authorize()
  const sindriManifest = await getSindriManifest()
  ...
  try {
    const proofResponse = await sindriClient.proveCircuit(
      circuitName,
      JSON.stringify(signals)
    )
    if (proofResponse.status === "Ready") {
      console.log(
        `Proof generated successfully, proof id: ${proofResponse.proof_id}`
      )
    } else {
      console.error(
